...
function renderStations() {
  ...
  const displayedStations = stations.slice(0, visibleCount);
  ...
  let html = `
        ...
        ${stations.length > visibleCount ? '<div class="load-more-container"><button id="load-more-btn" class="load-more-btn">Завантажити ще</button></div>' : ''}
        
        <footer class="app-footer">
          <div class="app-footer__copy">© <span id="current-year"></span> D3 Records. All Rights Reserved</div>
          <div class="app-footer__contacts">
            <span class="app-footer__label">Контакти:</span>
            <div class="app-footer__links">
              <a href="mailto:donchachok@gmail.com"><i data-lucide="mail"></i> donchachok@gmail.com</a>
              <a href="tel:+380957648567"><i data-lucide="phone"></i> +380957648567</a>
            </div>
          </div>
        </footer>
      `;
...
// Coffee Modal Logic
const coffeeBtn = document.getElementById('coffee-modal-btn');
const coffeeModal = document.getElementById('coffee-modal');
const coffeeClose = document.getElementById('coffee-modal-close');

if (coffeeBtn && coffeeModal && coffeeClose) {
  coffeeBtn.addEventListener('click', () => {
    coffeeModal.classList.add('modal--visible');
  });

  const closeModal = () => coffeeModal.classList.remove('modal--visible');

  coffeeClose.addEventListener('click', closeModal);
  coffeeModal.addEventListener('click', (e) => {
    if (e.target === coffeeModal) closeModal();
  });
}
